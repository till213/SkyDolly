...
SKY_FETCH_EGM           | OFF     | Downloads the earth gravity model EGM 2008 with a 5 minute resolution (around 18 MiB decompressed file size). The EGM file will then be placed into the `Resources` folder (in the `bin` output folder) at compile time
